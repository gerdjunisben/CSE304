        ...
        for thing in self.checkQueue:
            if(thing[0] ==0):
                ...
                if(res):
                    ...
                    thing[3].type = 'error'
                    raise Exception(f"Type mismatch between {thing[1].type} on line {thing[1].line} and {thing[2].type} on line {thing[2].line}")
            elif (thing[0] == 1):
                res = self.checkValid(thing[1],thing[2])
                if(res==None):
                    thing[3].type = 'error'
                    raise Exception(f"Type mismatch between {thing[1].type} on line {thing[1].line} invalid type for surrounding context")
                else:
                    thing[3].type = res
            elif (thing[0] == 3):
                ...
                if(res==None):
                    thing[2].type = 'error'
                    raise Exception(f"Type mismatch between returned value and header return {thing[1].type} on line {thing[1].line} and {thing[3].type} on line {thing[3].line}")
                else:
                ...
                if(res==None):
                    thing[4].type = 'error'
                    raise Exception(f"Invalid binary op involving {thing[1].type} on line {thing[1].line} and {thing[2].type} on line {thing[2].line} involving operation {thing[3]}")
                else:
                    thing[4].type = res
        self.checkQueue = []
